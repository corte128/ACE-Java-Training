/**
 * 
 */
package com.siriuscom.training;

import com.siriuscom.training.generic1.CountComparisons;
import java.util.*;
import java.lang.*;
import com.siriuscom.training.string.MonPal;
import com.siriuscom.training.date12.NewYorkDate;
import com.siriuscom.training.date13.FirstLastDayFinder;
/**
 * 
 * @author IEUser
 *
 */
public class HelloWorld {

	/**
	 * 
	 * @param args
	 */
	
	
	String msg;
	public static void main(String[] args) {

		//System.out.print(Day1Exercises.getDay(8, 3, 1916));
//		Day1ExerciseB.getGrade(78.5);
//		Day1ExerciseJ.printFibonacciNumbers();
		
//		Integer[] testArry = {23, 38, 2, 5, 43, 29, 29, 2, 5, 9};
//		Integer testElm = 9;
//		CountComparisons<Integer> integerComparisons = new CountComparisons<Integer>();
//		System.out.print(integerComparisons.compareGeneric(testArry, testElm));
		
//		System.out.print(MonPal.getNature("ljefo  wlkwe ljk fejlwjkl fjkl \"Jason Jacob Malassis\" lfdj o je ojds \"Fred Nemoy Might Styles\""));
//		System.out.print(NewYorkDate.getDateInNewYork());
		FirstLastDayFinder.printFirstLastDay(2);
	}

}
